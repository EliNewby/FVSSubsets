...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 17 15:23:57 2021
...
def findBestFVSSubsets(netName, numNodes, intersections = [['OutDegree','Distance','PRINCE','ModPRINCE','CheiRank','Cycles','SCC'],['PRINCE', 'ModPRINCE','CheiRank'],['ModPRINCE','CheiRank']], verbose = False, write = False, writePath = 'TopologicalValues.xlsx'):
    ...
    if(write):
        ...
        pd.DataFrame(FVSsubsets).to_excel(writer, sheet_name = 'FVS Subsets', header = False, index = False)
        topologyDF.to_excel(writer, sheet_name = 'Topology Metrics')
        intersectionDF.to_excel(writer, sheet_name = 'Intersection Metrics')
        writer.save()
    
    return topologyDF, intersectionDF, FVSsubsets
